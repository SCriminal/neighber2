...
- (void)resetViewWithModel:(ModelIntegralProduct *)model{
    ...
    //刷新view
    [self.scoreNum fitTitle:[NSString stringWithFormat:@"%.2f",model.price/100.0] variable:0];
    self.scoreNum.leftTop = XY(W(20),W(5));
    [self.score fitTitle:@"元" variable:0];
    self.score.leftBottom = XY(self.scoreNum.right + W(4),self.scoreNum.bottom-W(2));
    [self.remain fitTitle:[NSString stringWithFormat:@"剩%.f份",model.stockAmount]  variable:0];
    self.remain.rightBottom = XY(SCREEN_WIDTH - W(20),self.score.bottom);
