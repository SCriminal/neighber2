...
@interface SelectLayoutView ()<UIPickerViewDelegate,UIPickerViewDataSource>
@property (strong, nonatomic) NSMutableArray *aryDatas0;
@property (strong, nonatomic) NSMutableArray *aryDatas1;
@property (strong, nonatomic) NSMutableArray *aryDatas2;

@property (strong, nonatomic) UIPickerView *pickView;

@end

@implementation SelectLayoutView
...
- (NSMutableArray *)aryDatas0{
    if (!_aryDatas0) {
        ...
        for (int i =0; i<10; i++) {
            [_aryDatas0 addObject:[NSString stringWithFormat:@"%d室",i+1]];
        }
    }
    return _aryDatas0;
}
- (NSMutableArray *)aryDatas1{
    if (!_aryDatas1) {
        ...
        for (int i =0; i<10; i++) {
            [_aryDatas1 addObject:[NSString stringWithFormat:@"%d厅",i+1]];
        }
    }
    return _aryDatas1;
}
- (NSMutableArray *)aryDatas2{
    if (!_aryDatas2) {
        _aryDatas2 = [NSMutableArray new];
        for (int i =0; i<10; i++) {
            [_aryDatas2 addObject:[NSString stringWithFormat:@"%d卫",i+1]];
        }
    }
    return _aryDatas2;
}
    ...
    switch (component) {
        ...
        case 2:
        {
            if (row <= self.aryDatas2.count-1) {
                pickerLabel.text = self.aryDatas2[row];
            }
        }
            break;
        default:
...
- (void)confirmClick{
    if (self.blockSelect ) {
        self.blockSelect([self.pickView selectedRowInComponent:0]+1,[self.pickView selectedRowInComponent:1]+1,[self.pickView selectedRowInComponent:2]+1);
    }
    [self removeFromSuperview];
}
@end
