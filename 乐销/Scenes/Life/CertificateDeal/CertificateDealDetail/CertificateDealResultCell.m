...
@interface CertificateDealResultCell ()

@end

@implementation CertificateDealResultCell
#pragma mark 懒加载
- (UILabel *)problem{
    if (_problem == nil) {
...
- (void)resetCellWithModel:(ModelCertificateDealDetail *)model{
    ...
    //刷新view
    [self.problem fitTitle:UnPackStr(model.title) variable:W(270)];
    self.problem.leftCenterY = XY(self.labelBg.right+W(8),self.labelBg.centerY);
    
    
    
    
    [self.time fitTitle:[GlobalMethod exchangeTimeWithStamp:model.submitTime andFormatter:TIME_MIN_SHOW] variable:W(270)];
