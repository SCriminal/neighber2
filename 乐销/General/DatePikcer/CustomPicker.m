...
-(NSDate *)date
{
    ...
    NSString *month = [self.months objectAtIndex:([self selectedRowInComponent:NUM_MONTH])];
    
    //    NSInteger yearCount = self.years.count;
    
    NSString *year = [self.years objectAtIndex:([self selectedRowInComponent:NUM_YEAR])];
    
    NSInteger dayCount = self.days.count;
    NSString *day = @"1日";
    if (dayCount != 0 && self.datePickerMode != DatePickerYearMonthMode) {
        day = [self.days objectAtIndex:([self selectedRowInComponent:NUM_DAY]) % dayCount];
    }
    NSString *hour;
    if (self.datePickerMode == DatePickerHourMode) {
        hour = [self.hours objectAtIndex:([self selectedRowInComponent:NUM_HOUR])];
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (component == 0) {
        if ([self selectedRowInComponent:NUM_YEAR] == 0 && self.years.count != 1) {
            self.months = [self nameOfMinMonths];
        } else if ([self selectedRowInComponent:NUM_YEAR] == self.years.count - 1 && self.years.count != 1) {
        ...
        if (self.datePickerMode == DatePickerDateMode) {
            [pickerView reloadComponent:NUM_DAY];
        }
    }
    ...
    if (self.datePickerMode != DatePickerYearMonthMode) {
        ...
        NSInteger currert =[pickerView selectedRowInComponent:NUM_DAY] + 1;
        if (currert > [self daysCountWithSelDate] && self.datePickerMode == DatePickerDateMode) {
            [pickerView selectRow:[self daysCountWithSelDate] inComponent:NUM_DAY animated:NO];
        }
        if (component == 0 ||component == 1) {
            if(( self.years.count  == 1) && self.months.count == 1) {
                self.days = [self nameOfMinMaxDays];
            } else if (([self selectedRowInComponent:NUM_YEAR] == self.years.count - 1) && [self selectedRowInComponent:NUM_MONTH] == self.months.count - 1 ) {
                self.days = [self nameOfMaxDays];
            } else if (([self selectedRowInComponent:NUM_YEAR] == 0 ) && [self selectedRowInComponent:NUM_MONTH] == 0 ) {
                //&& self.months.count != 1
    ...
    if (self.datePickerMode == DatePickerHourMode) {
        if (component != NUM_HOUR) {
            if (([self selectedRowInComponent:NUM_YEAR] == self.years.count - 1) && ([self selectedRowInComponent:NUM_MONTH] == self.months.count - 1) && ([self selectedRowInComponent:NUM_DAY] == self.days.count - 1)) {
                self.hours = [self nameOfMaxHours];
                
            } else if (([self selectedRowInComponent:NUM_YEAR] == 0 ) && [self selectedRowInComponent:NUM_MONTH] == 0  && [self selectedRowInComponent:NUM_DAY] == 0) {
    ...
    if ([self.dvDelegate respondsToSelector:@selector(protocolDatePicker:didSelectedDate:)]) {
        [self.dvDelegate protocolDatePicker:self didSelectedDate:[self date]];
    }
...
- (void)selectDate:(NSDate *)date{
    ...
    [self selectRow: selectIndexPath.section
        inComponent: NUM_YEAR
           animated: YES];
    [self pickerView:self didSelectRow:selectIndexPath.row inComponent:NUM_YEAR];
    
    selectIndexPath = [self selectPathWithDate:date];
    
    [self selectRow: selectIndexPath.row
        inComponent: NUM_MONTH
           animated: YES];
    
    [self pickerView:self didSelectRow:selectIndexPath.row inComponent:NUM_MONTH];
    if (self.datePickerMode != DatePickerYearMonthMode) {
        CGFloat day = [self selectDayIndexWithDate:date];
        [self selectRow:day inComponent:NUM_DAY animated:YES];
        [self pickerView:self didSelectRow:day inComponent:NUM_DAY];
    }
    
    if (self.datePickerMode == DatePickerHourMode) {
        CGFloat hour = [self selectHourIndexWithDate:date];
        [self selectRow:hour inComponent:NUM_HOUR animated:YES];
        [self pickerView:self didSelectRow:hour inComponent:NUM_HOUR];
    }
